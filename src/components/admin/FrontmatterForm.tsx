  ...
  return (
    <div className="ap-meter">
      ...
      <span className="ap-meter-count" style={{ color }}>
        {value}<span className="ap-meter-cap">/{cap}</span>
      </span>
    </div>
  );
}
...
function TagBadgeInput({ tags, onChange }: { tags: string[]; onChange: (next: string[]) => void }) {
  ...
  const onBlur = () => { if (input.trim()) addTag(input); };
...
export function FrontmatterForm({ value, slug, content = "", images = [], onChange, onSlugChange }: FrontmatterFormProps) {
  ...
  const heroImage = String(value.heroImage ?? "");
  ...
  useEffect(() => { setPendingSlug(slug); }, [slug]);
  ...
  return (
    <div className="ap-form">
      ...
      <div className="ap-fm-section ap-fm-slug-section">
        <div className="ap-slug-wrap">
          <input
            ...
            value={pendingSlug}
            onChange={(e) => { setSlugLocked(true); setPendingSlug(e.target.value); setSlugError(null); }}
            onBlur={commitSlug}
            onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); void commitSlug(); } }}
            disabled={slugSaving}
          />
          <button
            type="button"
            className={`ap-slug-lock${slugLocked ? " locked" : ""}`}
            title={slugLocked ? "Unlock — slug auto-follows title" : "Lock — edit slug independently"}
            onClick={() => {
              if (slugLocked) { setSlugLocked(false); setPendingSlug(slug); }
      ...
      <div className="ap-fm-section">
        ...
        <Field label="Description" hint={<Meter value={description.length} cap={160} status={dStatus} />}>
          <textarea
      ...
      <div className="ap-fm-section">
        ...
        <Field label="Hero image" hint={<span className="ap-hero-label">Shown in post cards and page header</span>}>
          <input
            className="ap-input"
            placeholder="/images/blog/your-slug/banner.png"
            value={heroImage}
            onChange={(e) => set("heroImage", e.target.value || undefined)}
          />
          {heroImage && (
            <div className="ap-hero-preview">
              <img src={heroImage} alt="Hero preview" loading="eager" />
            </div>
          )}
          {images.length > 0 && (
            <div className="ap-hero-picker">
              {images.map((img) => (
                <button
                  key={img.url}
                  type="button"
                  className={`ap-hero-pick-btn${heroImage === img.url ? " ap-hero-active" : ""}`}
                  onClick={() => set("heroImage", heroImage === img.url ? undefined : img.url)}
                  title={img.name}
                >
      ...
      <div className="ap-fm-section">
        ...
        <Field
          ...
          hint={
            <span className="ap-tag-hint">
              <span style={{ color: tagColor }}>{tags.length} tag{tags.length !== 1 ? "s" : ""}</span>
        ...
        <div className="ap-toggle-row">
          <label className="ap-toggle">
            <input type="checkbox" checked={Boolean(value.draft)} onChange={(e) => set("draft", e.target.checked)} />
            <div className="ap-toggle-track"><div className="ap-toggle-thumb" /></div>
          ...
          <label className="ap-toggle">
            <input type="checkbox" checked={Boolean(value.featured)} onChange={(e) => set("featured", e.target.checked)} />
            <div className="ap-toggle-track"><div className="ap-toggle-thumb" /></div>
            <div className="ap-toggle-text">
