    ...
    <div className="ap-modal-backdrop" onClick={onCancel}>
      <div className="ap-modal ap-confirm" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="ap-modal-actions">
          <button className="ap-btn" onClick={onCancel}>Cancel</button>
...
export default function AdminApp() {
  ...
  const [rightWidth, setRightWidth] = useState(() => {
    try { return parseInt(localStorage.getItem("ap-right-width") ?? "460", 10) || 460; } catch { return 460; }
  });
  ...
  const startDrag = useCallback((e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      setRightWidth(newW);
      try { localStorage.setItem("ap-right-width", String(newW)); } catch { /* */ }
    };
    const onUp = () => {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setSaveState("idle");
    api.readPost(selection.collection, selection.slug).then((p) => {
      if (cancelled) return;
      setPost(p);
  ...
  const triggerSave = useCallback(
    async (immediate = false) => {
      ...
      const doSave = async () => {
        ...
        if (nextContent === lastSavedContent.current && nextFmStr === lastSavedFrontmatter.current) {
          dirtyRef.current = false;
          setSaveState("saved");
          return;
        }
        ...
        setSaveError(null);
        try {
          await api.savePost(selection.collection, selection.slug, {
          ...
          setSaveState("saved");
          try { previewFrameRef.current?.contentWindow?.location.reload(); } catch (_) {}
  ...
  const onCreate = async (values: {
    ...
    setPosts(fresh);
    setSelection({ collection: values.collection, slug: values.slug });
    setLeftTab("content");
    setRightTab("preview");
    setShowNew(false);
  };
  ...
  const onDelete = () => {
    ...
    setConfirmState({
      ...
      onConfirm: async () => {
        ...
        await api.deletePost(selection.collection, selection.slug);
  ...
  const onSlugChange = async (newSlug: string) => {
    ...
    await api.renamePost(selection.collection, selection.slug, newSlug);
    setPosts(await api.listPosts());
    setSelection({ collection: selection.collection, slug: newSlug });
  };

  const onSetCover = useCallback((url: string) => {
    if (!post) return;
    const fm = { ...liveFrontmatter.current };
    if (url) fm.heroImage = url; else delete fm.heroImage;
    const next = fm;
    liveFrontmatter.current = next;
    dirtyRef.current = true;
    setPost((p) => (p ? { ...p, frontmatter: next } : p));
  ...
  const currentCover = String(post?.frontmatter?.heroImage ?? "");

  const previewUrl = (() => {
    if (!selection) return "";
    if (selection.collection === "blog") return `/admin/preview/${selection.slug}/`;
    return `/${selection.slug}/`;
  })();

  return (
    <div className="ap-root" ref={rootRef} style={{ gridTemplateColumns: `320px 1fr 4px ${rightWidth}px` }}>
      <aside className="ap-sidebar">
        ...
        <div className="ap-post-list">
          {visiblePosts.map((p) => {
            const isSel = selection?.collection === p.collection && selection.slug === p.slug;
            const dateLabel = p.pubDate
              ? new Date(p.pubDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
              : null;
            return (
              <button
                key={`${p.collection}/${p.slug}`}
                className={`ap-post-item${isSel ? " on" : ""}`}
                onClick={() => setSelection({ collection: p.collection, slug: p.slug })}
              >
                <div className="ap-post-top">
                  <span className="ap-post-title">{p.title || p.slug}</span>
                  {p.featured && <span className="ap-featured-pill" title="Featured">★</span>}
                ...
                {p.tags.length > 0 && (
                  <div className="ap-post-tags">
                    {p.tags.slice(0, 3).map((t) => <span key={t} className="ap-post-tag">{t}</span>)}
                    {p.tags.length > 3 && <span className="ap-post-tag-more">+{p.tags.length - 3}</span>}
                  </div>
                )}
              </button>
            );
          ...
          <>
            <div className="ap-main-header">
              <div className="ap-tabs">
                <button className={leftTab === "content" ? "on" : ""} onClick={() => setLeftTab("content")}>
                  Content
                </button>
                <button className={leftTab === "frontmatter" ? "on" : ""} onClick={() => setLeftTab("frontmatter")}>
                  Frontmatter
                </button>
              </div>
              <div className="ap-main-actions">
                <SaveIndicator state={saveState} error={saveError} />
                <button className="ap-btn ap-btn-sm ap-btn-danger" onClick={onDelete}>Delete</button>
              </div>
            </div>
            <div className="ap-editor-wrap">
              <div ref={editorPaneRef} className="ap-editor-pane" style={{ display: leftTab === "content" ? "flex" : "none" }}>
                <CodeMirror
                  ref={editorRef}
                  key={`${selection.collection}/${selection.slug}`}
                  value={post.content}
                  theme={oneDark}
              ...
              <div className="ap-frontmatter-scroll" style={{ display: leftTab === "frontmatter" ? "block" : "none" }}>
                <FrontmatterForm
      ...
      <aside className="ap-right">
        <div className="ap-right-tabs">
          <button className={rightTab === "preview" ? "on" : ""} onClick={() => setRightTab("preview")}>Preview</button>
          <button className={rightTab === "components" ? "on" : ""} onClick={() => { setRightTab("components"); api.listComponents().then(setComponents); }}>Components</button>
          <button className={rightTab === "images" ? "on" : ""} onClick={() => setRightTab("images")}>Images</button>
          {selection && (
            <a className="ap-right-open" href={previewUrl} target="_blank" rel="noopener" title="Open preview">↗</a>
          )}
        </div>
...
function SaveIndicator({ state, error }: { state: SaveState; error: string | null }) {
  const label =
    ...
    state === "saved" ? "✓ Saved" :
    `✕ ${error ?? "Error"}`;
  const cls = state === "error" ? "err" : state === "saved" ? "ok" : state === "saving" ? "info" : "dim";
              ...
              {open && (
                <div className="ap-image-grid">
                  {g.images.map((img) => {
                    ...
                    return (
                    <div key={img.url} className={`ap-image-card${isCover ? " ap-image-card-cover" : ""}`}>
                      ...
                      <div className="ap-image-meta">
                        <span className="ap-image-name" title={img.name}>{img.name}</span>
